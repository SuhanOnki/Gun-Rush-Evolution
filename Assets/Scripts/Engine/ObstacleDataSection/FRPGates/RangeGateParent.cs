...
{
    public class RangeGateParent : GateParent
    {
        public static UnityEvent<float> OnRangeGateTrigger = new UnityEvent<float>();
                ...
                if (defenderState == defenderStateMax + 1)
                {
                    if (power < 0)
                    {
                        if (gateType == GateType.Plus)
                        {
                            ...
                            GameplayMaestro.Instance.StartEvent($"+{power} Range", Color.green);
                            GunBehavior.gunBehavior.Rotate();
                        }
                        else
                        {
                            GameplayMaestro.Instance.StartEvent($"{power} Range", Color.red);
                    ...
                    OnRangeGateTrigger?.Invoke(power);
